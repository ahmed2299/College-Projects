...
void WriteData() {                           // write the data after editting
...
void WriteDailyRep() {
 ...
 void IfClient() {
	...
	if (found) {
		...
		cout << "\t\t	5.your last 5 transactions\n\n";
		cout << "\t\t	6.close\n";
		cout << "\t\t";
 ...
 void IfEmp() {
	 ...
	 for (int i = 0; i < Num_of_E; i++) {
		 if (BankEmpsData[i].ID.find(ID) != -1 && BankEmpsData[i].password.find(pass) != -1) {
			 found = true;
			 EmpNum = i;
			 cout << endl << endl << endl << endl << endl << endl;
			 cout << "\t\t you have logged in succesfully" << endl;
	 ...
	 if (found) {
		 while (true) {
			 ...
			 if (order == 1) {                            // to show the data
				 ...
				 cout << "Your balance : " << User[ClientNum].deposite << " LE" << endl;
				 cout << "ID : " << User[ClientNum].ID << endl;
				 cout << endl << endl << endl;
				 cout << "Do you want to back to the menu (y/n)?" << endl;
				 cin >> Back;
